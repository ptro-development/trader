...
def get_samples_correlation_chains(
        mixin, trade_data, distance, sample_epoch):
    ...
    for m in mixin:
        if m[1] == sample_epoch and len(chain) == 0:
            chain = [m]
            continue
        if len(chain) > 0 and m[0][0] < chain[0][0][0] + distance:
            chain.append(m)
        else:
            if len(chain) > 0:
...
def next_trade_data_analyze(
    ...
    print network
    import json
    previous_samples_path = "previous_samples.json"
    print "Loading previous tree samples from %s" % previous_samples_path
    with open(previous_samples_path, "r") as fd:
        previous_samples = json.load(fd)
    ...
    new_file_path = "testing_data_examples/testing_data_examples/small-data-2"
    ...
    print "Running samples matching."
    sample_size = len(previous_samples[0]["sample_data"])
    required_correlation = previous_samples[0]["required_correlation"]
    ...
    # skeleton_gain_tree = load_pickle("skeleton_gain_tree.pickle")
    # skeleton_loss_tree = load_pickle("skeleton_loss_tree.pickle")

    """
    layers_counts = get_tree_layers_counts(skeleton_tree)
    print "Skeleton tree layers counts", layers_counts
    layers_counts_extended = get_tree_layers_counts_extended(skeleton_tree)
    print "Skeleton tree layers counts", layers_counts_extended
    """

    """
    new_trade_data_path = "next_rescaled_trade_data.pickle"
    print "Loading next rescaled trade data from " + str(new_trade_data_path)
    with open(new_trade_data_path, "r") as fd:
        new_trade_data = pickle.load(fd)

    import json
    matched_samples_path = "matched_samples.json"
    print "Loading new matched samples from " + str(matched_samples_path)
    with open(matched_samples_path, "r") as fd:
        matched_samples = json.load(fd)
    """

    print "Build new sample chain."
    ...
    """
    print "Running new trade sequence on old tree."
    top_leafs_keys = [
        l.get_key() for l in previous_tree.get_root().get_next_leafs()]
    trails = TreeKeysTrailsCollection(previous_tree, top_leafs_keys)
    """

    print "Running new trade sequence on old skeleton tree."
    # TODO
    skeleton_tree = None
    tree_copy = copy.deepcopy(skeleton_tree)
    trails = TreeKeysTrailsCollection(skeleton_tree)

    # allowed_range = 0
    # TODO
    new_trade_sample_chain = None
    for bit in new_trade_sample_chain.get_chain():
        # trails.append(bit[1])
        trails.append_only_if_inactive(bit[1])
        # trails.compute_probability_and_delete_key(
        #    bit[1], bit[0], population.get_tree_depth())
        # trails.compute_probability_delete_key_within_range(
        #    bit[1], bit[0],
        #    population.get_tree_depth(), allowed_range, sample_size)
        # trails.compute_probability_delete_euclidean_closest_key(
        trails.compute_probability_delete_euclidean_closest_key_extended(
            bit[1], bit[0], previous_samples)
    # compute_amount_to_buy(network)
    trails.normalize_amount_to_buy()
    # capital = 1000.0
    gain, loss, investment, partial_investments, no_limit_gain, no_limit_loss, no_limit_investment = trails.get_overall_gain_for_estimated_trades_amounts_extended(new_trade_data)  # noqa
    # TODO
    gain_trails = TreeKeysTrailsCollection(previous_gain_tree)
    loss_trails = TreeKeysTrailsCollection(previous_loss_tree)
    compute_probability_and_delete_key(
        gain_loss_trade_sequences=(new_gain_chain, new_loss_chain),
        gain_loss_tree_trails=(gain_trails, loss_trails),
        gain_loss_samples=(previous_gain_samples, previous_loss_samples))
    print " Gain/loss_from_limited_investment: %s,%s" % (gain, loss) + \
        " Limited_investment: " + str(investment) + \
        " Partial_investment: " + str(partial_investments) + \
        " Gain/loss_from_no_limit_investment: %s,%s" % (no_limit_gain, no_limit_loss) + " No_limit_investment: " + str(no_limit_investment)  # noqa

    """
    tree_copy = copy.deepcopy(previous_tree)

    print "Running new trade sequence on old tree ..."
    top_leafs_keys = [
        l.get_key() for l in previous_tree.get_root().get_next_leafs()]
    past_tree_trails = TreeKeysTrailsCollection(previous_tree, top_leafs_keys)
    for bit in new_trade_sample_chain.get_chain():
        past_tree_trails.append(bit[1])
        past_tree_trails.delete_key(bit[1], bit[0], 20)
        print "\n"
    """

    original_tree_counts = get_tree_layers_counts(tree_copy)
    modified_tree_counts = get_tree_layers_counts(trails.get_tree())
    ...
    counts_percentages_coverage = [
        100 - (modified_tree_counts[i] / (original_tree_counts[i] / 100.0))
        for i in range(0, len(original_tree_counts))]
    print "Coverage in percentage:", counts_percentages_coverage

    known, unknown = get_tree_layers_counts_extended(tree_copy)
...
def get_populated_tree_with_sequence(tree, tree_depth, sequence):
    whole_sequence_past_tree_trails = TreeKeysTrailsCollection(tree)
    for bit in sequence:
        # whole_sequence_past_tree_trails.append(bit[1])
    ...
    return whole_sequence_past_tree_trails.get_tree()
...
def recalculate_samples_attributes(data):
    ...
    map(lambda x: x.update({"sample_attributes": analyse_sample_attributes_extended(x["sample_data"])}),  # noqa
        data["sorted_samples_plus"])
    map(lambda x: x.update({"sample_attributes": analyse_sample_attributes_extended(x["sample_data"])}),  # noqa
        data["sorted_samples_minus"])
...
def relation(
    ...
    one_sample = data["sorted_samples_plus"][0]
    ...
    # TODO: also minus samples should loaded
    facts = build_correlation_facts(
        data["sorted_samples_plus"] + data["sorted_samples_minus"], trade_data)
    print "Build facts' time correlation map with samples which do not overlap and most occurrent samples considered first."  # noqa
    facts_time_cor_map = get_time_correlation_map_3(facts, sample_size)
    ...
    reset_tree_counters(gain_tree.get_root())
    reset_tree_counters(loss_tree.get_root())
    ...
    clean_gain_samples = get_clean_samples(
        data["sorted_samples_plus"], previous_gain_samples_epochs)
    clean_loss_samples = get_clean_samples(
        data["sorted_samples_minus"], previous_loss_samples_epochs)
    ...
    save_pickle(
        ...
        get_populated_tree_with_sequence(
            gain_tree, gain_tree_depth, new_gain_sequence))
    loss_tree_depth = len(get_tree_layers_counts(loss_tree))
    save_pickle(
        "previous_loss_tree.pickle",
        get_populated_tree_with_sequence(
            loss_tree, loss_tree_depth, new_loss_sequence))
...
def train_network(
    ...
    print "Gain tree layers counts", gain_layers_counts, gain_layers_counts_extended  # noqa
    print "Loss tree layers counts", loss_layers_counts, loss_layers_counts_extended  # noqa
    ...
    networks_population = NetworksPopulation(
        size=10,
        # network_layers_layout=[5, 8, 3, 1],
        network_layers_layout=[10, 5, 1],
        # network_layers_layout=[4, 7, 5, 1],
        input_values_ranges=[
            [0, 1], [0, 1], [0, depth-1], [0, records_count], [0, records_count],  # noqa
            [0, 1], [0, 1], [0, depth-1], [0, records_count], [0, records_count]],  # noqa
